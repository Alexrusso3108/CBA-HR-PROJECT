...
export default function HRPerformance() {
  ...
  // Performance & goals are still local for now — show empty gracefully
  const allReviews = [];
  const allGoals = [];
  ...
  const empData = useMemo(() => activeEmps.map(e => {
    ...
    const leaves = leaveApplications.filter(l => l.employeeId === e.id && l.status === 'approved').length;
  ...
  }), [activeEmps, allReviews, allGoals, leaveApplications, departments]);
    ...
    <Layout title="HR — Performance Overview">
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        {[
          ...
          { label: 'Active Employees', value: activeEmps.length, icon: Users, color: '#0ea5e9', bg: '#f0f9ff' },
        ].map(s => {
          const Icon = s.icon;
          return (
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table className="data-table">
            <thead>
              <tr>
                <th>Employee</th><th>Department</th><th>Review Status</th>
                <th>Self Rating</th><th>Manager Rating</th>
                <th>Goal Progress</th><th>Goals</th><th>Leaves Taken</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={8}><div className="empty-state" style={{ padding: 32 }}><p>Loading…</p></div></td></tr>
              ) : empData.length === 0 ? (
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: '#94a3b8' }}>{e.employeeCode}</div>
                        </div>
                      </div>
                    </td>
